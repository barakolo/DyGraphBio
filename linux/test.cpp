...
int main()
{
    ...
    // Init random devices.
    random_device rd;
	uniform_int_distribution<int> dist(0, nm.size()-1);
	mt19937 engine2(rd());

    // Setting the prior nodes.
    list<Node*> prior;
    while (prior.size()!= PRIOR_SIZE) {
        prior.push_back(nm[100+prior.size()]);
        continue;
        uint32_t cnode = dist(engine2);
        if (!nm[cnode]->in_hlist())
            prior.push_back(nm[cnode]);
    }
	g.set_prior(prior);
	g2.set_prior(prior);
    ...
    // Average diff + touches.
    double avg_diff = 0.0;
    double avg_touches = 0;
    double avg_touches2 = 0;
    uint32_t kos_cnt = 0; 
	...
	for (uint32_t i = 200; i < 10200; i++) {
        ...
        if (i < PRIOR_SIZE)
            continue;
		//if (std::find(prior.begin(), prior.end(), nm[i]) != prior.end())
        //    continue;
		...
		cout << "DynFP: First time scratch was: " << ts_fp << " seconds." << endl;
		...
		cout << "PropStatic: First time scratch was: " << ts_ps << " seconds." << endl;
		cout << "PropStatic: Current neto-time for dels: " << time_span2.count() << " seconds." << endl;
	
        uint32_t nt = g3.get_nt();
        avg_touches = ((avg_touches * kos_cnt) + nt) / (double)(kos_cnt + 1);
        cout << "avg touches for PropStatic:" << avg_touches << endl;
		/*
        g3.set_prior(prior);
        g3.set_eps(1e-8);
		t1 = high_resolution_clock::now();
        ...
        double cur_diff = diff(scores, scores2, nm.size());
        avg_diff = ((avg_diff * kos_cnt) + cur_diff) / (double)(kos_cnt + 1);
        cout << "cur diff:" << cur_diff << endl;
        cout << "avg diff:" << avg_diff << endl;
        
        uint32_t nt2 = g.get_nt();
        avg_touches2 = ((avg_touches2 * kos_cnt) + nt2) / (double)(kos_cnt + 1);

        cout << "avg touches for DynFP:" << avg_touches2 << endl;
        kos_cnt++;
	}
